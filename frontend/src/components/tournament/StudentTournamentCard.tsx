...
import { formatDateTime, getDaysUntilDate } from "@/lib/dateUtils";
...
const StudentTournamentCard: React.FC<StudentTournamentCardProps> = ({
  ...
  const [
    registerStudentToTournament,
    { loading: registerLoading, error: registerError },
  ] = useRegisterStudentToTournamentMutation();
  ...
  // formatDate function removed - using formatDateTime from dateUtils instead

  // getDaysUntilStart function removed - using getDaysUntilDate from dateUtils instead

  // Check if student is already registered
  const isStudentRegistered = studentId
    ? tournament.participants.includes(studentId)
    : false;
  const daysUntilStart = getDaysUntilDate(tournament.date);
  ...
  return (
    <Card
      className={`h-full flex flex-col cursor-pointer transition-all duration-200 hover:shadow-lg border-2 ${
        isActive
    ...
    >
      ...
      <CardFooter className="flex gap-2 pt-4">
        ...
        {String(tournament.status).toUpperCase() === "OPENING" &&
          !isStudentRegistered &&
          !isRegistered && (
            <Button
              onClick={(e) => {
