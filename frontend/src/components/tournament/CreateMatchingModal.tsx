...
import { Users, Shuffle, Target, Clock, Trophy } from "lucide-react";
...
const CreateMatchingModal: React.FC<CreateMatchingModalProps> = ({
  ...
  return (
    <Modal
      ...
      description={`Generate match rooms for: ${tournamentName}`}
      size="lg"
    >
      <ModalContent className="space-y-6">
        ...
        <div
          className={`rounded-lg p-4 ${
            participantStudents.length > 0 ? "bg-blue-50" : "bg-red-50"
          }`}
        >
            ...
            <div>
              ...
              {participants.length > 0 && participantStudents.length === 0 && (
                <p className="text-xs text-red-600 mt-1">
                  Debug: {participants.length} participant IDs found but no
                  matching students
                </p>
              )}
            </div>
        ...
        {participantStudents.length === 0 && (
          <div className="bg-red-50 rounded-lg p-4">
            <p className="text-sm text-red-800">
              <strong>Cannot create matches:</strong> No students are registered
              for this tournament. Students need to register first before
                ...
                >
                  ...
                  <div className="space-y-2">
                    {room.participants.map((participant, index) => (
                      <div
                        key={`${room.id}-participant-${participant.id}-${index}`}
