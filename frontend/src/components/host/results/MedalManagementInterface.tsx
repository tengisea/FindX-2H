    ...
    const handleSaveMedalAssignments = async () => {
      ...
      try {
        ...
        if (result.data?.updateMedalAssignments?.success) {
          ...
          showError(
            `Медалийн олгох мэдээлэл шинэчлэхэд алдаа гарлаа: ${result.data?.updateMedalAssignments?.message || "Unknown error"
            }`
          );
        }
      } catch (error) {
        ...
        showError(
          `Медалийн олгох мэдээлэл шинэчлэхэд алдаа гарлаа: ${error instanceof Error ? error.message : "Unknown error"
          }`
        );
        ...
        <div className="bg-primary/5 rounded-xl p-6 border border-primary/20">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-lg font-semibold text-primary">
              Төлөв: {olympiad.status === "MEDALS_PREVIEW" ? "Медал урьдчилан харах" : "Дууссан"}
            </h4>
            <span
              className={`px-3 py-1 rounded-full text-sm font-medium ${olympiad.status === "MEDALS_PREVIEW"
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {classTypes.map((classType) => (
              <div
                key={classType.id}
                className={`bg-card rounded-xl border-2 p-6 cursor-pointer transition-all duration-200 ${selectedClassType === classType.id
                          ...
                          {["gold", "silver", "bronze", "top10"].map(
                            (medalType) => (
                              <button
                                ...
                                className={`w-14 h-14 rounded-xl flex items-center justify-center text-2xl font-bold transition-all duration-200 transform hover:scale-110 active:scale-95 cursor-pointer ${medalAssignments[selectedClassType]?.[
                                  medalType
                                ]?.includes(student.studentId)
                                  ? `${getMedalColorTable(
                                    medalType
                                  )}  scale-105`
                                  : "bg-gradient-to-br from-gray-100 to-gray-200 text-gray-600  hover:from-gray-200 hover:to-gray-300 hover:text-gray-800  hover:scale-105"
                                  }`}
                              >
                                {getMedalIcon(medalType)}
                              </button>
                          ...
                          {["gold", "silver", "bronze", "top10"].map(
                            (medalType) => (
                              <button
                                ...
                                className={`w-10 h-10 rounded-full flex items-center justify-center text-lg transition-all duration-200 ${medalAssignments[selectedClassType]?.[
                                  medalType
                                ]?.includes(studentId)
          ...
          <div className="bg-card rounded-xl border border-border p-6">
            <div className="text-center">
              <h4 className="text-lg font-semibold text-foreground mb-2">
                Медалийг эцэслэхэд бэлэн үү?
              </h4>
              <p className="text-muted-foreground mb-4">
                Эцэслэгдсэний дараа медалийн хуваарилалт сурагчдад илгээгдэж, дахин өөрчлөх боломжгүй.
              </p>
              <button
                onClick={() => setShowFinalizeConfirm(true)}
                disabled={isFinalizing}
                className={`px-8 py-3 rounded-xl transition-all duration-200 flex items-center space-x-2 mx-auto shadow-lg ${isFinalizing
              ...
              >
                {isFinalizing ? (
                  <>
                    ...
                    <span>Медалийг эцэслэж байна..."</span>
                  </>
                ) : (
                  <>
                    <svg
                    ...
                    <span>Бүх медалийг эцэслэх</span>
                  </>
                )}
              </button>
            </div>
        ...
        >
          ...
          <ModalFooter>
            ...
            <Button
              ...
              disabled={isFinalizing}
              className={`flex-1 sm:flex-none border-0 ${isFinalizing
                ? "bg-gray-400 text-gray-600 cursor-not-allowed"
