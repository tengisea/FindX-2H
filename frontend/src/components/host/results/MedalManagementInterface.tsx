...
export const MedalManagementInterface: React.FC<MedalManagementInterfaceProps> = ({
    olympiad,
    classTypes,
    onFinalizeMedals,
    ...
    const selectedClassTypeData = classTypes.find(ct => ct.id === selectedClassType);

    const handleMedalAssignment = (classTypeId: string, medalType: "gold" | "silver" | "bronze", studentId: string, assigned: boolean) => {
        setMedalAssignments(prev => ({
            ...prev,
            [classTypeId]: {
                ...prev[classTypeId],
                [medalType]: assigned
                    ? [...(prev[classTypeId]?.[medalType] || []), studentId]
                    : (prev[classTypeId]?.[medalType] || []).filter((id: string) => id !== studentId)
            }
        }));
    };
    ...
    const handleSaveMedalAssignments = async () => {
        ...
        const assignments = Object.entries(medalAssignments).map(([classTypeId, medals]: [string, any]) => ({
            classTypeId,
            gold: medals.gold || [],
            ...
            <div className="bg-primary/5 rounded-xl p-6 border border-primary/20">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                        ...
                        {olympiad.status === "MEDALS_PREVIEW" ? "Ready for Review" : "Finalized"}
                    </span>
                </div>
                ...
                <div className="bg-card rounded-xl border border-border p-6">
                    <div className="flex items-center justify-between mb-6">
                        <h4 className="text-lg font-semibold text-foreground">
                            Medal Assignments - Grade {selectedClassTypeData.classYear.replace('GRADE_', '')}
                        </h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        {["gold", "silver", "bronze"].map((medalType) => (
                            <div key={medalType} className={`rounded-xl p-6 border-2 ${getMedalColor(medalType)}`}>
                                <div className="text-center">
                                    <div className="text-3xl mb-2">{getMedalIcon(medalType)}</div>
                                    <h5 className="font-semibold capitalize mb-2">{medalType} Medal</h5>
                    ...
                    <div className="space-y-4">
                        <h5 className="font-medium text-foreground">Assign Medals to Students</h5>
                        <div className="space-y-2">
                            {selectedClassTypeData.participants?.slice(0, 10).map((studentId: string, index: number) => (
                                <div key={studentId} className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        {["gold", "silver", "bronze"].map((medalType) => (
                                            <button
                                                key={medalType}
                                                onClick={() => handleMedalAssignment(
                                                    selectedClassType, 
                                                    medalType as "gold" | "silver" | "bronze", 
                                                    studentId, 
                                                    !medalAssignments[selectedClassType]?.[medalType]?.includes(studentId)
                                                )}
                ...
                <div className="bg-card rounded-xl border border-border p-6">
                    <div className="text-center">
                        ...
                        <p className="text-muted-foreground mb-4">
                            Once finalized, medal assignments will be sent to students and cannot be changed.
                        </p>
                        <button
