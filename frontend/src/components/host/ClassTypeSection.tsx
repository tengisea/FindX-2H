  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h3 className="text-2xl font-bold text-gray-900">Class Types & Questions</h3>
        {!editingOlympiad && (
          <button
            type="button"
      ...
      {classTypes.map((classType, classTypeIndex) => (
        <div key={classTypeIndex} className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div>
              ...
              <select
                ...
                onChange={(e) => onUpdateClassType(classTypeIndex, 'classYear', e.target.value as ClassYear)}
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h5 className="text-lg font-semibold text-gray-900">Questions</h5>
              {!editingOlympiad && (
                <button
                  type="button"
            ...
            <div className="space-y-4">
              {classType.questions.map((question, questionIndex) => (
                <div key={questionIndex} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-xl">
                  <div className="flex-1">
                    <input
                      type="text"
                      value={question.questionName}
                      onChange={(e) => onUpdateQuestion(classTypeIndex, questionIndex, 'questionName', e.target.value)}
                  ...
                  <div className="w-24">
                    <input
                      ...
                      onChange={(e) => {
                        const value = e.target.value;
                        const numValue = value === '' ? 0 : parseInt(value) || 0;
                        onUpdateQuestion(classTypeIndex, questionIndex, 'maxScore', numValue);
                      }}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder="Score"
                      disabled={!!editingOlympiad}
                  ...
                  {!editingOlympiad && classType.questions.length > 1 && (
                    <button
                      type="button"
                      onClick={() => onRemoveQuestion(classTypeIndex, questionIndex)}
                      className="text-red-600 hover:text-red-700 p-2 rounded-lg hover:bg-red-50 transition-colors"
