...
export const ManageResults: React.FC<ManageResultsProps> = ({
  ...
  const refreshMedalPreviews = async () => {
    ...
    // Check if olympiad is in CLOSED status before previewing medals
    const olympiadData = olympiads.find((o) => o.id === selectedOlympiad);
    if (
      olympiadData?.status !== "CLOSED" &&
      olympiadData?.status !== "MEDALS_PREVIEW" &&
      olympiadData?.status !== "FINISHED"
    ) {
      console.warn("Olympiad must be CLOSED before previewing medals");
      return;
    }
  ...
  React.useEffect(() => {
    if (
      ...
      // Check if olympiad is in correct status before refreshing medal previews
      const olympiadData = olympiads.find((o) => o.id === selectedOlympiad);
      if (
        olympiadData?.status === "CLOSED" ||
        olympiadData?.status === "MEDALS_PREVIEW" ||
  ...
  }, [viewMode, selectedOlympiad, olympiads]);
        ...
        <div className="flex items-center justify-center min-h-[600px]">
          <div className="text-center max-w-md mx-auto">
            <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-6">
              <svg
                className="w-10 h-10 text-[#ff8400]"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <p className="text-gray-600 text-base leading-relaxed">
              Дээрх цэснээс сонгож, түүний үр дүнг харах болон удирдах
              боломжтой.
            </p>
          </div>
        </div>
          ...
          <div className="space-y-6 min-h-[600px]">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="px-6 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors flex items-center space-x-2 h-12 min-w-[120px] border border-gray-200"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
  ...
  return (
    <div className="w-full min-h-screen">
      <div
        className="bg-white rounded-2xl sm:rounded-3xl shadow-xl border border-gray-200 p-6 lg:p-8 relative overflow-hidden min-h-[800px]"
        style={{
          boxShadow:
      ...
      >
        ...
        <div className="mb-8 px-6">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            <div className="flex-1 max-w-md">
              ...
              >
                <SelectTrigger className="w-full h-12 px-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff8400] focus:border-transparent text-base bg-white text-gray-900">
        ...
        {selectedOlympiad && (
          <div className="mb-8 px-6">
            <div className="flex flex-wrap gap-3">
              <button
                onClick={() => setViewMode("overview")}
                className={`px-6 py-3 rounded-lg text-base font-medium transition-colors min-w-[140px] h-12 ${
                  viewMode === "overview"
                    ? "bg-[#ff8400] text-white shadow-md"
        ...
        {/* Main Content */}
        <div className="relative px-6 min-h-[600px]">{renderContent()}</div>
      </div>
    </div>
  );
};
