...
const StudentTournamentBracketPage = () => {
    ...
    return (
      <div className="text-center py-12 bg-red-50 border border-red-200 rounded-lg m-8">
        ...
        <p className="text-red-600">
          {tournamentError?.message || matchRoomsError?.message}
        </p>
      </div>
    );
  }
  ...
  // Check if student is registered for the tournament
  const isStudentRegistered = studentId
    ? tournament.participants.includes(studentId)
    : false;

  // If student is not registered, show join tournament message
        ...
        <Card className="w-full max-w-md">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-600 mb-4">
              You are not registered for this tournament yet. Join to
              participate and view the bracket.
            </p>
  ...
  const getPlayerName = (playerId: any, playerData: any = null) => {
    ...
    if (!student) {
      student = students.find(
        (s: any) => s.id.toString() === searchId.toString()
      );
    }
    ...
    if (student) {
      ...
      const partialMatch = students.find(
        (s: any) =>
  ...
  const getPlayerAvatar = (playerId: any, playerData: any = null) => {
    ...
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(
      `Student ${String(searchId).slice(-4)}`
    )}&background=random&color=fff&size=40`;
  };
                      ...
                      <div className="space-y-3">
                        {[...piWard.students]
                          ...
                          .map((student: any, studentIndex: number) => (
                            <div
                              key={`student-${index}-${studentIndex}-${
                                student.place || studentIndex
                              }`}
                                    ...
                                    <div className="w-12 h-12 rounded-full overflow-hidden bg-blue-100 flex items-center justify-center">
                                      <img
                                        ...
                                        onError={(e) => {
                                          ...
                                          const studentName = getPlayerName(
                                            student.studentId
                                          );
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-600 mb-6">
                Matches haven't been created for this tournament yet. Please
              ...
              <div className="flex items-center justify-between min-w-[1000px] space-x-8 border rounded-lg px-12 py-6">
                ...
                <div className="flex flex-col space-y-4">
                  ...
                  {matches
                    ...
                    .map((match, index) => (
                      <div
                        key={match.id || `quarter-match-${index}`}
                        className="relative"
                      >
                        <div
                          className="bg-gray-50 rounded-lg p-4 min-w-[220px] border-2 border-gray-200 hover:border-blue-300 transition-colors cursor-pointer"
                          onClick={() =>
                            router.push(
                              `/tournament/${tournamentId}/duel/${match.id}`
                            )
                          }
                        >
                          {/* Player A */}
                              ...
                              <div className="flex flex-col">
                                ...
                                {(match as any).slotAData?.school && (
                                  <span className="text-xs text-gray-500">
                                    {((match as any).slotAData as any).school}
                                  </span>
                                )}
                              </div>
                            </div>
                              ...
                              <div className="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center overflow-hidden">
                                <img
                                  ...
                                  onError={(e) => {
                                    ...
                                    target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(
                                      initials
                                    )}&background=ef4444&color=fff&size=40`;
                                  }}
                                />
                              </div>
                              <div className="flex flex-col">
                                ...
                                {(match as any).slotBData?.school && (
                                  <span className="text-xs text-gray-500">
                                    {((match as any).slotBData as any).school}
                                  </span>
                                )}
                              </div>
                            </div>
...
export default StudentTournamentBracketPage;
