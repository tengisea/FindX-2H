...
import { MutationResolvers, AssignTasksToChallengeInput } from "@/types/generated";

export const assignTasksToChallenge: MutationResolvers["assignTasksToChallenge"] = async (
  _: unknown,
  { input }: { input: AssignTasksToChallengeInput }
) => {
  try {
    ...
    const invalidTasks = tasks.filter(task => 
      task.topic !== challenge.topic || task.difficulty !== challenge.difficulty
    );
    ...
    return {
      ...
      difficulty: updatedChallenge.difficulty as any,
      ...
      status: updatedChallenge.status as any,
      tasks: (updatedChallenge.tasks as any[]).map(task => ({
      ...
      createdAt: (updatedChallenge as any).createdAt,
      updatedAt: (updatedChallenge as any).updatedAt,
    };
  } catch (error) {
