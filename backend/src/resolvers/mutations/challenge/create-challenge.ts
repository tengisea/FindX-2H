...
export const createChallenge: MutationResolvers["createChallenge"] = async (
  ...
  try {
    ...
    console.log(
      `📊 Challenge params: topic=${input.topic}, difficulty=${input.difficulty}, classType=${input.classType}, taskCount=${input.taskCount}`
    );

    // Validate task count (limited to 3 for OpenAI API limits)
    if (input.taskCount < 1 || input.taskCount > 3) {
      throw new GraphQLError(
        "Task count must be between 1 and 3 (limited due to OpenAI API constraints)"
      );
    }
    ...
    if (input.taskDifficultyDistribution) {
      const {
        ...
        medium = 0,
        hard = 0,
      } = input.taskDifficultyDistribution;
      const totalDistributedTasks =
        (easy as any) + (medium as any) + (hard as any);

      if (totalDistributedTasks !== input.taskCount) {
        throw new GraphQLError(
          `Task difficulty distribution mismatch: taskCount is ${input.taskCount} but difficulty distribution adds up to ${totalDistributedTasks}. Please ensure they match.`
        );
      }
    }
    ...
    if (!input.challenger || input.challenger.trim() === "") {
      throw new GraphQLError(
        "Challenger ID is required and must be a valid ObjectId"
      );
    }
    if (!input.opponent || input.opponent.trim() === "") {
      throw new GraphQLError(
        "Opponent ID is required and must be a valid ObjectId"
      );
    }

    // Validate topic is appropriate for the grade level
    if (
      !AIMappers.validateTopicForGrade(
        input.topic as any,
        input.classType as any
      )
    ) {
      const gradeNumber = AIMappers.getGradeNumber(input.classType as any);
      ...
      if (gradeNumber <= 3) {
        ...
        allowedTopics =
          "MATH, ENGLISH, GEOGRAPHY, HISTORY, EARTH_SCIENCE, PHYSICS, CHEMISTRY, BIOLOGY, COMPUTER_SCIENCE, ALGORITHMS, DATA_STRUCTURES";
      } else {
        allowedTopics = "All topics are allowed";
      }

      throw new GraphQLError(
        `Topic ${input.topic} is not appropriate for Grade ${gradeNumber}. Allowed topics for this grade: ${allowedTopics}`
      );
    }

    // Generate tasks automatically based on challenge parameters
    const taskCount = input.taskCount;
    const piPointsPerTask = input.piPoints
      ? Math.floor(input.piPoints / taskCount)
      : 10; // Distribute pi points across tasks
    ...
    if (input.answerFormatDistribution) {
      ...
      const mapChallengeFormatToAnswerFormat = (
        format: string
      ): AnswerFormat => {
        switch (format) {
          case "MULTIPLE_CHOICE":
            return AnswerFormat.MultipleChoice;
          case "SHORT_TEXT":
            return AnswerFormat.ShortText;
          default:
            return AnswerFormat.MultipleChoice;
        }
      };
      ...
      const mapToAnswerFormatDistribution = (
        dist: AnswerFormatDistribution
      ): any => ({
        easy: dist.easy
          ? mapChallengeFormatToAnswerFormat(dist.easy)
          : undefined,
        medium: dist.medium
          ? mapChallengeFormatToAnswerFormat(dist.medium)
          : undefined,
        hard: dist.hard
          ? mapChallengeFormatToAnswerFormat(dist.hard)
          : undefined,
      });

      const answerFormatDistribution = mapToAnswerFormatDistribution(
        input.answerFormatDistribution
      );

      generatedTasks =
        await TaskGeneratorService.generateMultipleTasksWithFormatOptions(
          {
            topic: input.topic as any,
            type: TaskType.Challenge,
            classType: input.classType as any,
            piPoints: piPointsPerTask,
            taskCount: taskCount,
            difficultyDistribution:
              (input.taskDifficultyDistribution as any) || {
                // Distribute tasks based on the challenge difficulty
                easy:
                  input.difficulty === "EASY"
                    ? taskCount
                    : Math.floor(taskCount * 0.3),
                medium:
                  input.difficulty === "MEDIUM"
                    ? taskCount
                    : Math.floor(taskCount * 0.5),
                hard:
                  input.difficulty === "HARD"
                    ? taskCount
                    : Math.floor(taskCount * 0.2),
              },
          },
          answerFormatDistribution
        );
    ...
    console.log(
      `🤖 Auto-generating answers for ${generatedTasks.length} tasks...`
    );
    for (const task of generatedTasks) {
      try {
        // Map model types to GraphQL types
        const { topic, classType } =
          AnswerGeneratorService.mapModelToGraphQLTypes(
        ...
        console.error(
          `⚠️ Failed to generate answer for task ${task.title}:`,
          answerError
        );
    ...
    console.log(
      `🎉 Challenge created successfully with ID: ${challenge._id} and ${generatedTasks.length} tasks with answers`
    );
