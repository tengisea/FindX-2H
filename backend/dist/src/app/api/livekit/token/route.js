...
export async function POST(req) {
    try {
        ...
        if (!process.env.LIVEKIT_API_KEY || !process.env.LIVEKIT_API_SECRET) {
            return NextResponse.json({ error: "LiveKit API credentials not configured" }, {
                status: 500,
                headers: {
        ...
        if (!studentId || !roomName) {
            return NextResponse.json({ error: "studentId and roomName are required" }, {
                status: 400,
                headers: {
                    'Access-Control-Allow-Origin': '*',
                    'Access-Control-Allow-Methods': 'POST, OPTIONS',
        ...
        if (typeof studentId !== "string" || typeof roomName !== "string") {
            return NextResponse.json({ error: "studentId and roomName must be strings" }, {
                status: 400,
                headers: {
                    'Access-Control-Allow-Origin': '*',
                    'Access-Control-Allow-Methods': 'POST, OPTIONS',
