...
function allocatePiPoints(results, totalPiPoints) {
    return results.map(r => {
        ...
        return Object.assign(Object.assign({}, r), { points });
    });
}
export const createPiWard = async (_, { tournamentId }) => {
    ...
    // Round-ийг эрэмбэлэх
    const roundOrder = { Final: 1, Semifinal: 2, Quarterfinal: 3 };
    ...
    if (!(finalMatch === null || finalMatch === void 0 ? void 0 : finalMatch.winner) || !(finalMatch === null || finalMatch === void 0 ? void 0 : finalMatch.loser))
    ...
    semiMatches.forEach(m => {
        if (m.loser)
            topPlayers.push({ studentId: m.loser.toString(), place: 3 + topPlayers.filter(p => p.place >= 3).length });
    });
    // 5-8 байр (Quarterfinal loser)
    const quarterMatches = sortedMatches.filter(m => m.round === "Quarterfinal");
    quarterMatches.forEach(m => {
        if (m.loser)
            topPlayers.push({ studentId: m.loser.toString(), place: 5 + topPlayers.filter(p => p.place >= 5).length });
    });
    // Pi оноо оноох
